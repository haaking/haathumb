...
/**
 * haathumb is simplest way to call timthumb script.
 * via htaccess rewrite rules or web.config rewrite rules
 * any question ben@haakan.com
 *
...
$a='c';
$zc=0;
$data = explode('/',$_GET['query']);
$size = explode('x',$data[0]);

if ($size[0]>0)                            $timthumb_string.='&w='.$size[0];
if ($size[1]>0)                            $timthumb_string.='&h='.$size[1];
...
if (in_array('colorize'     ,$data)){
    ...
    $key++;
    $color = hex2rgb($data[$key]);
    $rgba = $color;
    $rgba[3]=127;
    $items = count($rgba);
    $rgbad = implode(',',$rgba);
    if (strlen ($data[$key]) ==6 && $items == 4 && is_numeric($rgba[0]) && is_numeric($rgba[1]) && is_numeric($rgba[2]) && is_numeric($rgba[3]))    $filter.='|5,'.$rgbad;
}
if (in_array('background'     ,$data)){
    $key = array_search('background',$data);
    $key++;
    $color = hex2rgb($data[$key]);
    $rgba = $color;
    $items = count($rgba);
    $rgbad = implode(',',$rgba);
    if (strlen ($data[$key]) ==6 && $items == 3 && is_numeric($rgba[0]) && is_numeric($rgba[1]) && is_numeric($rgba[2]))    $timthumb_string.='&cc='.$data[$key];
}
if (in_array('transparent'   ,$data))        $timthumb_string.='&ct=1';
if (in_array('center'       ,$data))         $a='c';
if (in_array('left'         ,$data))         $a='l';
if (in_array('right'        ,$data))         $a='r';
if (in_array('bottom'       ,$data))         $a='b';
if (in_array('top'          ,$data))         $a='t';
if (in_array('top-left'     ,$data))         $a='tl';
if (in_array('top-right'    ,$data))         $a='tr';
if (in_array('bottom-right' ,$data))         $a='br';
if (in_array('bottom-left'  ,$data))         $a='bl';
if (in_array('moved'        ,$data))         $zc='2';
if (in_array('cropped'      ,$data))         $zc='1';
if (in_array('lower'        ,$data))         $zc='3';
if (in_array('maximum'      ,$data))         $quality=100;
if (in_array('medium'       ,$data))         $quality=50;
if (in_array('high'         ,$data))         $quality=90;
if (in_array('low'          ,$data))         $quality=25;
if (in_array('sharpen'      ,$data))         $timthumb_string.='&s=1';
if (in_array('negative'     ,$data))         $filter.='1|';
if (in_array('grayscale'    ,$data))         $filter.='2|';
if (in_array('blur'         ,$data))         $filter.='8|';
if (in_array('emboss'       ,$data))         $filter.='7|';
if (in_array('edges'        ,$data))         $filter.='6|';
...
if(! defined('JPEG_IS_PROGRESSIVE') )		define ('JPEG_IS_PROGRESSIVE', $_GET['progressive']);					// Define if a created jpeg image is progressive
